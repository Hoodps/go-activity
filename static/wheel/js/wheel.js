...
$(function(){
	...
	var ROOT = HOST + '/static/css/main/2015/lottery/res/';
	var URL = {
	    'mobi' : '/wheel/input_mobi',
	    'lottery' : '/wheel/run?referer=UNKNOW',
	    'myAward' : '/wheel/award_list',
	    'getAward' : '/wheel/get_award',
	    'putAward' : '/wheel/put_award',
	    'list' : '/wheel/index_data',
	    'rule' : '/wheel/activity_rule'
	};
	var OPT = {
		good : {
			1 : [1,"特等奖"],
	    ...
	    switch (type){
	    	case "rule" : {
	    		_tpl =  '<div class="box2">'+
		    				...
		    				'<article>'+
		    					...
		    					'<p><em>活动时间</em> 2015年8月1日~9月30日</p>'+
		    					'<p><em>活动规则</em> </p>'+
		    					'<p>1、请确保输入启动大转盘的号码与本次登录“广东移动10086”客户端的手机号码一致。所有奖品赠送前，将复核启动大转盘号码是否在活动期间成功登录“广东移动10086”客户端，未符合活动规则的获奖记录将视为无效，不予赠送。</p>'+
		    					'<p>2、客户每天均有1次转盘机会，如对转到的奖品不够满意，只要不领取，次日可继续转大奖，高颜值的P7手机等着您！活动期间奖品限领取1次。</p>'+
		    					'<p>3、成功领取的奖品可点击“获奖查询”。不领取或领取失败视为未中奖。</p>'+
		    					'<p>4、符合活动规则获奖的客户，我司将在活动结束后的50个工作日内，安排手机奖品的外呼配送和流量奖品的赠送，流量到账时会有短信告知。4G随心王客户暂无法享受流量赠送。</p>'+
		    					'<p>5、请保持手机状态正常，赠送流量或外呼配送时如遇号码状态异常将无法赠送或登记，则视为放弃。</p>'+
                                '<p>6、每个获奖手机号码仅对应一个身份证号，同一身份证号多次获奖手机奖品仅配送一部。如发现有刷奖等作弊行为，则中奖作废。</p>'+
		    				'</article>' +
	    				'</div>' + 
	    	...
	    	case "my" : {
	    		if(data.mobi){
	    			...
	    			var list = "";
	    			for(var i in d){
	    				list += '<tr><td>'+d[i].award_name+'</td><td>'+d[i].join_datetime+'</td><td></td></tr>';
	    			}
		    		_tpl =  '<h1><b>'+d[i].mobi+'</b>'+'<span>如非本月“广东移动10086” 登录号码将无法获得奖品</span></h1>'+
	    					'<table>'+
	...
	$btnStart.on("click",function(){
    	...
    	var url = URL["lottery"];
    	$t.addClass("locked");
        $.post(url,{'mobi':$mobi.val(),'activity_id':act_id,'tk':$("#tk").val()},function(r){
            if(r && r.code==0){
            	if(r.data.level > 0){
					startLottery(r.data.level,function(){
						...
						$mask.find(".btn-cancel").attr({"id":r.data.id,"aid":r.data.lottery_award_id});
					})
            	}else{
	...
	$mask.on("click",".btn-submit",function(){
	    ...
	    if(checkMobile($mobile)){
	    	...
	    	$t.addClass("locked");
	        $.post(url,{'mobi':mobi,'activity_id':act_id,'tk1':$("#tk1").val()},function(r){
	            $t.removeClass("locked");
	            if(r && r.code==0){

	            	$mobi.val(r.data.mobi);
	...
	$mask.on("click",".btn-accept",function(){
    	...
    	var aid = $t.attr("aid");
    	$t.addClass("locked");
        $.post(url,{'mobi':mobi,'id':id,'lotterys_award_id':aid,'activity_id':act_id,'tk2':$("#tk2").val()},function(r){
            $t.removeClass("locked");
            if(r && r.code==0){
            	$mask.find(".close").trigger("click");
                alert("恭喜您，奖品领取成功！");
